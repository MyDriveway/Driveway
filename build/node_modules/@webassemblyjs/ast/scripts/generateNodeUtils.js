...
function params(fields) {
  ...
  return mapProps(fields).map(function (field) {
    return "" + typeSignature(field) + optionalDefault(field);
  }).join(",");
}
...
function generate() {
  ...
  iterateProps(definitions, function (typeDefinition) {
    stdout.write("\n      export function " + lowerCamelCase(typeDefinition.name) + " (\n        " + params(filterProps(typeDefinition.fields, function (f) {
      return !f.constant;
    })) + "\n      ): " + typeDefinition.name + " {\n\n        " + assertParams(filterProps(typeDefinition.fields, function (f) {
      return !f.constant;
  ...
  unionTypes.forEach(function (unionType) {
    stdout.write("\n      export const is" + unionType + " = (node: Node) => " + mapProps(definitions).filter(function (d) {
      return d.unionType && d.unionType.includes(unionType);
    }).map(function (d) {
      return "is" + d.name + "(node) ";
    }).join("||") + ";\n\n");
  });
  ...
  stdout.write("\n    export const unionTypesMap = {" + mapProps(definitions).filter(function (d) {
    ...
    return "\"" + t.name + "\": [" + t.unionType.map(quote).join(",") + "]\n";
  }) + "};\n      ");
